...
The `Pane` and `Card` components are one of the most important components in MolecularUI.
...
The `Box` component is a UI Primitive component that is used as the base of many components in MolecularUI.
...
Many components within MolecularUI use the `Box` component instead of a element such as `div` or `button`.
...
In MolecularUI most of the time you don’t deal with CSS classes through the `className` property.
Instead, you write CSS properties directly onto your MolecularUI components —
in most cases it will just work.

_If you know what you are doing and do need to use CSS directly, be aware that mixing
MolecularUI with regular CSS might give you unexpected results._
...
```jsx
// import { Pane, Text } from 'molecular-ui'
...
## Override single properties when using MolecularUI components

Most components in MolecularUI use a `Pane` or `Box` under the hood.
To add spacing, or layout properties, pass the properties straight to the component:

```jsx
// import { Pane, Button, Text, Heading } from 'molecular-ui'
...
In most cases in MolecularUI you can use the `height` property on components to change the size
of a component. The text styles and padding will adjust based on the height you pass.

```jsx
// import { Pane, Button, TextInput } from 'molecular-ui'
...
Within MolecularUI we call this the major scale.

#### MolecularUI exports a utility called `majorScale` to help conform to the 8px major scale.
...
```jsx
// import { majorScale, Button } from 'molecular-ui'
...
#### MolecularUI exports a utility called `minorScale` to help conform to the 4px minor scale.
...
```jsx
// import { minorScale, Button } from 'molecular-ui'
...
Currently there is no opinionated way to construct responsive layouts in MolecularUI.
...
In the case when you need to pass properties to a MolecularUI component based on the viewport,
