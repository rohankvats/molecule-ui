...
There is a need to be intentional and functional with color use. Although the MolecularUI color system has a bit of overlap with the Segment brand.
...
The color system in MolecularUI is located in the theme and is used throughout the theme.
There is no real dependency on any of the colors directly within components.
Components always access a theme color or property through a get function. For example, `theme.getTextColor` is a required function in the MolecularUI theme, theme.colors is not a required property.
