...
Previously, the way we shipped icons in `cereal-ui@^4` would significantly bloat bundle sizes. Even if you only
used 1 icon from Evergreen, you would ship all 400+ icons to your end-users.

In `cereal-ui@^4.26.0` we introduced a new way to import icons that would lead to _some amount_ of tree-shaking.

Finally, in `cereal-ui@^5.0.0` we have full tree-shaking support. This does impact the way you import icons (`import { Icon } from 'cereal-ui'`) and pass
...
updates `import { Icon } from 'cereal-ui'`.

```sh static
npx jscodeshift -t node_modules/cereal-ui/codemods/4.28.1.0-4.29.0/replace-icon-imports.js --parser=tsx --extensions=js,ts,tsx <your file target> --dry --print
```
...
```diff static
- import { Icon } from 'cereal-ui'
+ import { CogIcon } from 'cereal-ui'
...
+ import { Button, CogIcon } from 'cereal-ui'
